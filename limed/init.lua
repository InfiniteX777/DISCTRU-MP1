...
function love.textinput(key)
	...
	if focus then
		local txt = focus.text

		focus.text = txt:sub(1,txt:len()-focus.focusPosition)..key..
			txt:sub(txt:len()-focus.focusPosition+1,txt:len())

		focus.focusBlinkTimer = 0
	end
end
...
function love.keypressed(key,...)
	...
	if focus then
		if key == "return" then
			game:releaseFocus(true)
		elseif key == "backspace" then
			local txt = focus.text

			if txt:len() > focus.focusPosition then
				focus.text = txt:sub(1,txt:len()-focus.focusPosition-1)..
					txt:sub(txt:len()-focus.focusPosition+1,txt:len())
			end
		...
		elseif key == "delete" then
			...
			if txt:len() > txt:len()-focus.focusPosition then
				focus.text = txt:sub(1,txt:len()-focus.focusPosition)..
					txt:sub(txt:len()-focus.focusPosition+2,txt:len())
				focus.focusPosition = focus.focusPosition-1
			end
...
function love.mousepressed(...)
	if game.focus and not game.focus.hover then
		game:releaseFocus()
	end
