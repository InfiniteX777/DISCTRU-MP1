...
local Quad = Image:class("Quad",4){
	...
	draw = function(self,super,x,y,angle,sx,sy,ox,oy,...)
		if self.image then
			local x,y,angle,sx,sy,ox,oy = x or 0,y or 0,angle or 0,sx or 1,sy or 1,ox or 0,oy or 0
			local w,h = self.width,self.height
			self.frame = self.frame or self:get(self.x,self.y,w,h)
			x,y,ox,oy = x+w/2*sx,y+h/2*sy,ox+w/2,oy+h/2
			...
			love.graphics.draw(self.image,self.frame,x,y,angle,sx,sy,ox,oy,...)
